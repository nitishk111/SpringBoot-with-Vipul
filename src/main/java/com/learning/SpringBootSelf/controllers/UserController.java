...
public class UserController {
    ...
    public ResponseEntity<?> updateEntry(@RequestBody User user){
        ...
        try {
            service.updateEntry(userName , user);
            return new ResponseEntity<>("Record with username= " + userName + " updated.", HttpStatus.OK);
        }catch (Exception e){
            return new ResponseEntity<>("No record available with username= "+userName, HttpStatus.NOT_FOUND);
        }
    }
    ...
    public ResponseEntity<?> deleteById(){
        ...
        String userName= auth.getName();
        try{
            service.deleteEntry(userName);
            return new ResponseEntity<>("Record with username= "+userName +" deleted", HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<String>("No Record Available of userName= "+userName, HttpStatus.NO_CONTENT);
        }
